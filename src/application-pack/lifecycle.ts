...
export async function generatePackForJob(args: {
  ...
  if (!store.isStepCompleted(jobId, 'companyResearch')) {
    ...
    try {
      ...
      const template = loadTemplate('company-research', templatesRoot);
      const rendered = renderTemplate(template, { webSearchResults, jd: jdText });
  ...
  if (!store.isStepCompleted(jobId, 'painPoints')) {
    ...
    try {
      const template = loadTemplate('pain-points', templatesRoot);
  ...
  if (!store.isStepCompleted(jobId, 'mapping')) {
    ...
    try {
      ...
      const template = loadTemplate('mapping', templatesRoot);
      const rendered = renderTemplate(template, {
        companyProfile: companyProfileForPrompts,
        jd: jdText,
  ...
  if (!store.isStepCompleted(jobId, 'experienceBullets')) {
    ...
    try {
      const template = loadTemplate('experience-bullets', templatesRoot);
      const rendered = renderTemplate(template, {
        jd: jdText,
        painPoints: painPointsText,
        companyProfile: companyProfileForPrompts,
      ...
      store.writeOutFile(jobId, 'experience-bullets.raw.txt', result);
      store.writeOutFile(jobId, 'experience-bullets.extracted.txt', extractExperienceBullets(result));
  ...
  if (!store.isStepCompleted(jobId, 'summary')) {
    ...
    try {
      const template = loadTemplate('summary', templatesRoot);
      const rendered = renderTemplate(template, {
        jd: jdText,
        painPoints: painPointsText,
        companyProfile: companyProfileForPrompts,
  ...
  if (!store.isStepCompleted(jobId, 'coverLetter')) {
    ...
    try {
      const template = loadTemplate('cover-letter', templatesRoot);
      const rendered = renderTemplate(template, {
        jd: jdText,
        summary: summaryText,
        painPoints: painPointsText,
  ...
  if (!store.isStepCompleted(jobId, 'review')) {
    ...
    try {
      const template = loadTemplate('review', templatesRoot);
      const rendered = renderTemplate(template, {
        companyProfile: companyProfileForPrompts,
        painPoints: painPointsText,
        mapping: mappingText,
  ...
  // Review FAIL does not auto-regenerate — callers/UI handle regenerate explicitly.
  return { reviewFailed };
}
